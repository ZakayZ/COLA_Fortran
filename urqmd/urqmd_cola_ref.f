...
!     22=BB elastic, 26=MB elastic, 38=MM elastic
      implicit none
      integer proc_id
      is_elastic_proc = (proc_id.eq.13 .or. proc_id.eq.17 .or.
     &     proc_id.eq.19 .or. proc_id.eq.22 .or. proc_id.eq.26 .or.
     &     proc_id.eq.38)
      return
      end
...
! ---------------------------------------------------------------------------
     ...
     &     parts, ini_parts, pdga, pdgb, pza, pzb, eout, snnout,
...
!     loop over all timesteps
            ...
            if (nct.gt.0) then
               coll_loop: do
                  ...
                  do
                     ...
                     if (cti2(k).gt.0 .and.
     .                   abs(sqrts(cti1(k),cti2(k))-ctsqrts(k)).gt.1d-3)
     &                   then
     ...
     &                    ctcolfluc(k))
                     ...
                     if (CTOption(17).eq.0) then
                        if (nexit.eq.0) then
                           if (cti1(k).ne.cti1sav .or. cti2(k).ne.cti2sav)
     &                         then
                              cti1(k) = cti1sav
                              cti2(k) = cti2sav
                           endif
