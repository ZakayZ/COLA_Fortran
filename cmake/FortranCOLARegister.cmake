...
function (_register_fortran_source_filters)
    cmake_parse_arguments(
        ARG "" "USER_MODULE_NAME;MODULE_NAME;FILTER_TYPE;OUTPUT_DIR" "TYPE_NAMES" ${ARGN}
    )
    ...
    foreach (TYPE_NAME IN LISTS ARG_TYPE_NAMES)
        ...
        configure_template(
            TEMPLATE
            "${COLA_Fortran_TEMPLATES_DIR}/cpp/${CPP_WRAPPER_NAME}.hh.in"
            OUTPUT
        ...
        configure_template(
            TEMPLATE
            "${COLA_Fortran_TEMPLATES_DIR}/cpp/${CPP_WRAPPER_NAME}.cc.in"
            OUTPUT
            ${GENERATED_CPP_PATH}
            VARS
            CLASS_NAME
        ...
        configure_template(
            TEMPLATE
            "${COLA_Fortran_TEMPLATES_DIR}/fortran/${FILTER_TYPE_LOWER}_wrapper.f90.in"
            OUTPUT
...
function (register_fortran_source)
    ...
    get_filename_component(
        FORTRAN_FILE_ABS "${ARG_FORTRAN_FILE}" ABSOLUTE BASE_DIR "${CMAKE_CURRENT_SOURCE_DIR}"
    )
    ...
    if (NOT USER_MODULE_NAME)
        set(GENERATED_FORTRAN_SOURCE_FILES
            ""
            PARENT_SCOPE
        )
        set(GENERATED_HEADER_SOURCE_FILES
            ""
            PARENT_SCOPE
        )
        set(GENERATED_CPP_SOURCE_FILES
            ""
            PARENT_SCOPE
        )
        set(COLA_FILTERS
    ...
    set(GENERATED_FORTRAN_SOURCE_FILES
        "${_GENERATED_FORTRAN_SOURCE_FILES}"
        PARENT_SCOPE
    )
    set(GENERATED_HEADER_SOURCE_FILES
        "${_GENERATED_HEADER_SOURCE_FILES}"
        PARENT_SCOPE
    )
    set(GENERATED_CPP_SOURCE_FILES
        "${_GENERATED_CPP_SOURCE_FILES}"
        PARENT_SCOPE
    )
endfunction ()
...
function (add_cola_fortran_library MODULE_NAME)
    ...
    foreach (SOURCE IN LISTS ARG_SOURCES)
        register_fortran_source(
            MODULE_NAME "${MODULE_NAME}" FORTRAN_FILE "${SOURCE}" OUTPUT_DIR "${OUTPUT_DIR}"
        )
    ...
    string(REPLACE ";" ".hh>\n#include <${MODULE_NAME}/" INCLUDE_FORTRAN_WRAPPER_CLASSES
                   "${ALL_COLA_FILTERS}"
    )
    set(INCLUDE_FORTRAN_WRAPPER_CLASSES
        "#include <${MODULE_NAME}/${INCLUDE_FORTRAN_WRAPPER_CLASSES}.hh>"
    )

    configure_template(
        TEMPLATE
        "${COLA_Fortran_TEMPLATES_DIR}/cpp/FortranModule.hh.in"
        OUTPUT
        "${OUTPUT_DIR}/${MODULE_NAME}/${MODULE_NAME}Module.hh"
        VARS
        MODULE_NAME
        "${MODULE_NAME}"
        COMMA_SEPARATED_CLASS_NAMES
        ${COMMA_SEPARATED_CLASS_NAMES}
        INCLUDE_FORTRAN_WRAPPER_CLASSES
        ${INCLUDE_FORTRAN_WRAPPER_CLASSES}
    )

    configure_template(
        TEMPLATE
        "${COLA_Fortran_TEMPLATES_DIR}/cpp/FortranModule.cc.in"
        OUTPUT
    ...
    add_library(
        ${MODULE_NAME} SHARED ${ARG_SOURCES} ${ALL_GENERATED_FORTRAN_FILES}
                              ${ALL_GENERATED_CPP_FILES} "${OUTPUT_DIR}/${MODULE_NAME}Module.cc"
    )

    set_target_properties(
        ${MODULE_NAME} PROPERTIES Fortran_MODULE_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR}/modules
